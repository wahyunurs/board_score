<!-- Edit modal -->
<div id="edit-modal-{{ $mediaPartner->id }}" tabindex="-1" aria-hidden="true"
    ...
    <div class="relative p-4 w-full max-w-md max-h-full">
        ...
        <div class="relative bg-white rounded-lg shadow light:bg-gray-700">
            ...
            <div class="p-6 md:p-8">
                <form method="POST" action="{{ route('manage-media-partners.update', $mediaPartner->id) }}"
                    enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="id" value="{{ $mediaPartner->id }}">

                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="name"
                                class="block mb-2 text-sm font-medium text-gray-900 light:text-white">Nama Media
                                Partner</label>
                            <input type="text" name="name" id="name-{{ $mediaPartner->id }}"
                                class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                                required value="{{ old('name', $mediaPartner->name) }}">
                        </div>

                        <div class="col-span-2">
                            <label for="description"
                                class="block mb-2 text-sm font-medium text-gray-900 light:text-white">Deskripsi</label>
                            <textarea name="description" id="description-{{ $mediaPartner->id }}" rows="3"
                                class="bg-gray-50 border text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">{{ old('description', $mediaPartner->description) }}</textarea>
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <div class="mb-2">
                                <img src="{{ asset('storage/images/media-partner/' . $mediaPartner->logo) }}"
                                    alt="{{ $mediaPartner->name }}" class="w-20 h-20 object-contain border rounded">
                            </div>
                            <input type="file" name="logo" id="logo-{{ $mediaPartner->id }}"
